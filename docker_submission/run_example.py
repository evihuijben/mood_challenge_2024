...
if __name__ == "__main__":
    ...
    try:
        # Removed read-only flag for the sake of downloading AlexNet and LPIPS model weights
        # docker_str = (
        ...
        docker_str = (
            f"docker run {gpu_str}-v {toy_input_dir}:/mnt/data "
            f"-v {output_dir}:/mnt/pred mood_example sh /workspace/run_pixel_{region}.sh /mnt/data /mnt/pred"
        )
        print(docker_str)
    ...
    try:
        ...
        docker_str = (
            f"docker run {gpu_str}-v {toy_input_dir}:/mnt/data "
            f"-v {output_dir}:/mnt/pred mood_example sh /workspace/run_sample_{region}.sh /mnt/data /mnt/pred"
        )
        print(docker_str)
        ret = subprocess.run(docker_str.split(" "), check=True,)
    except Exception:
